...
export default function ProblemSection() {
  return (
    <section id="problem" className="relative border-t border-line py-12 md:py-16 lg:py-[88px]">
      <div className="mx-auto w-full max-w-[1280px] px-6">
        ...
        <div className="grid items-start gap-8 md:gap-12 lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)] lg:gap-16">
          ...
          <div className="lg:sticky lg:top-[104px]">
            ...
            <p className="mt-3 max-w-[520px] text-[1.05rem] leading-[1.65] text-dim md:mt-4">
          ...
          <div className="flex flex-col gap-3 md:gap-4">
            {PAINS.map(({ Icon, title, body }) => (
              <div
                key={title}
                className="flex gap-4 rounded-[16px] border border-line bg-surface p-5 md:gap-5 md:p-6"
              >
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-[12px] border border-[rgba(255,107,107,0.25)] bg-danger-wash text-danger md:h-11 md:w-11">
                  <Icon className="h-5 w-5 shrink-0 md:h-[22px] md:w-[22px]" />
                </div>
                <div className="min-w-0">
                  <h3 className="mb-1.5 font-display text-[1.1rem] font-semibold leading-[1.15] tracking-[-0.01em] md:text-[1.15rem]">
                    {title}
                  </h3>
                  <p className="text-[0.95rem] leading-[1.6] text-dim md:text-[0.98rem]">{body}</p>
                </div>
              </div>
            ))}
          </div>
